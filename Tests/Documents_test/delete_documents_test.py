...
class TestDeleteDocuments:
    def test_delete_document(self, directory_id_my_documents):
    ...
    def test_delete_table(self, directory_id_my_documents):
    ...
    def test_delete_presentation(self, directory_id_my_documents):
    ...
    def test_multiple_delete_document(self, directory_id_my_documents):
        # создание документов, множественное удаление документов
        response = Documents.create_document(directory_id_my_documents, "Документ1.docx")
        assert_status_code(response)
        document1_id = response.json()[0]["Id"]

        response = Documents.create_document(directory_id_my_documents, "Документ2.docx")
        assert_status_code(response)
